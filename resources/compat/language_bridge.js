/* Calibre-only language compatibility bridge.
 *
 * Scope is intentionally narrow:
 *   1) repair obviously wrong source-language metadata before LingKuma TTS;
 *   2) localize LingKuma technical tag metadata to the SOURCE language
 *      without changing stored tag strings or vendored upstream UI code.
 *
 * This file must not own tooltip layout, theme, glass, highlighting, sentence
 * extraction, or WordExplosion lifecycle.
 */
(() => {
  'use strict';
  if (!globalThis.__LINGKUMA_CALIBRE_READER__) return;
  if (globalThis.__LK_CALIBRE_LANGUAGE_BRIDGE_INSTALLED__) return;
  globalThis.__LK_CALIBRE_LANGUAGE_BRIDGE_INSTALLED__ = true;
  ...
  function strongSourceLanguage(word, sentence = '') {
    ...
    if (hasHan(w) || hasHan(context)) {
      // Kanji-only tokens are ambiguous in isolation; page text resolves the
      // common Chinese-vs-Japanese case without asking AI or changing upstream.
  ...
  function persistSourceLanguage(word, language) {
    ...
    // Never create vocabulary records for whole TTS sentences.
    ...
    try {
      if (globalThis.highlightManager?.wordDetailsFromDB) {
        ...
        if (current.language !== language) {
          globalThis.highlightManager.wordDetailsFromDB[key] = { ...current, language };
        }
      }
    } catch (_) {}
    try {
      globalThis.chrome.runtime.sendMessage({ action: 'updateWordLanguage', word: term, language }, () => {});
    } catch (_) {}
  }

  function installSourceLanguageRepair() {
    const originalFetch = globalThis.fetchLanguageDetection;
    if (typeof originalFetch === 'function' && !originalFetch.__lkCalibreLanguageWrapped) {
      ...
      wrapped.__lkCalibreLanguageWrapped = true;
      wrapped.__lkCalibreOriginal = originalFetch;
      globalThis.fetchLanguageDetection = wrapped;
    }

    // Second, contract-level guard: if stale stored metadata still causes an
    // upstream TTS call with lang="en" for CJK text, correct only that message.
    // This does not alter the TTS implementation itself.
    const runtime = globalThis.chrome?.runtime;
    if (runtime?.sendMessage && !runtime.sendMessage.__lkCalibreLanguageWrapped) {
      ...
      const wrappedSend = function(...args) {
        ...
        if (message && typeof message === 'object' && ['playAudio', 'playLocal', 'playTTS', 'playEdgeTTS'].includes(message.action)) {
      ...
      wrappedSend.__lkCalibreLanguageWrapped = true;
  ...
  function localizeTagControls(root, sourceLanguage) {
    ...
    root.querySelectorAll?.('input.tag-input').forEach(input => { input.placeholder = labels.addTagPlaceholder; });
  }

  function capturedTooltipRoots() {
    const capture = globalThis.__LK_CALIBRE_SHADOW_CAPTURE__;
    if (!capture?.get) return [];
    const roots = [];
    document.querySelectorAll('lingkuma-tooltip-root').forEach(host => {
      const root = capture.get(host);
  ...
  function applyTooltipLocalization() {
    refreshQueued = false;
    for (const root of capturedTooltipRoots()) {
      const word = String(root.querySelector('.Notes')?.textContent || '').trim();
  ...
  function installTooltipLocalization() {
    tooltipObserver = new MutationObserver(queueLocalization);
    tooltipObserver.observe(document.documentElement, { childList: true, subtree: true, characterData: true });
    queueLocalization();
  }
  ...
  globalThis.__LK_CALIBRE_LANGUAGE_BRIDGE__ = {
  ...
  console.info('[LingKuma calibre] language compatibility bridge installed');
})();
